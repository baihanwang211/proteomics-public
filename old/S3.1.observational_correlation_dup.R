### this script estimates the observational correlations between non-one-to-one matached Olink reagents and SOMAmers
...
# find how many with at least one pair < 0.4

n_aptamer$one_0.4_normal <- FALSE

for (i in 1:nrow(n_aptamer)) {
  df <- overlap_cor_protein_dup[overlap_cor_protein_dup$uniprot_id==n_aptamer$uniprot_id[i],]
  if (max(df$rho_olink_soma_normal)>0.4) {
    n_aptamer$one_0.4_normal[i] <- TRUE
  }
}

sum(n_aptamer$one_0.4_normal)

n_aptamer$one_0.4_non_normal <- FALSE

for (i in 1:nrow(n_aptamer)) {
  df <- overlap_cor_protein_dup[overlap_cor_protein_dup$uniprot_id==n_aptamer$uniprot_id[i],]
  if (max(df$rho_olink_soma_non_normal)>0.4) {
    n_aptamer$one_0.4_non_normal[i] <- TRUE
  }
}

sum(n_aptamer$one_0.4_non_normal)
...
ggsave("",plot_rho_olink_soma_protein_dup,width=14,height=6)
...
ggsave("",plot_pairwise_cor_soma,width=14,height=6)
...
n_protein <- data.frame(table(overlap_cor_aptamer_dup$uniprot_id))
names(n_protein) <- c("uniprot_id","n_protein")
table(n_protein$n_protein)

# merge with original dataset

overlap_cor_aptamer_dup <- merge(overlap_cor_aptamer_dup,n_protein,by="uniprot_id")

# find how many with at least one pair < 0.4

n_protein$one_0.4 <- FALSE

for (i in 1:nrow(n_protein)) {
  df <- overlap_cor_aptamer_dup[overlap_cor_aptamer_dup$uniprot_id==n_protein$uniprot_id[i],]
  if (max(df$rho_olink_soma_normal)>0.4) {
    n_protein$one_0.4[i] <- TRUE
  }
}

sum(n_protein$one_0.4)
...
ggsave("",plot_rho_olink_soma_aptamer_dup,width=14,height=6)
...
plot_pairwise_cor_olink <- ggplot(pairwise_cor_olink, aes(x=rho)) + 
  ...
  ylim(0,6) +
  annotate("text", x = 0.45, y = 4, label = "Median rho = 0.31", size = 12/.pt) +
  geom_vline(aes(xintercept=median(rho)),color="black", linetype="dashed", linewidth=1) +
...
ggsave("",plot_pairwise_cor_olink,width=7,height=6)
