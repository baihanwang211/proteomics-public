...
somascan_non_ANML_log <- read.csv("somascan_non_ANML_log.csv")

##### load overlapping proteins

overlap_cor <- read.csv("overlap_cor.csv")

##### annotate proteins with meta files are obtained from CKB data access platform
...
mean_log_soma_non_ANML <- data.frame(colMeans(somascan_non_ANML_log[, 2:ncol(somascan_non_ANML_log)]))
names(mean_log_soma_non_ANML) <- "mean_log_soma_non_ANML"
mean_log_soma_non_ANML$somascan_id <- row.names(mean_log_soma_non_ANML)
overlap_cor <- merge(overlap_cor,mean_log_soma_non_ANML,by="somascan_id")
...
skewness_soma_non_ANML <- data.frame(skewness(somascan_non_ANML_log[,2:ncol(somascan_non_ANML_log)]))
names(skewness_soma_non_ANML) <- c("skewness_soma_non_ANML")
skewness_soma_non_ANML$somascan_id <- row.names(skewness_soma_non_ANML)
overlap_cor <- merge(overlap_cor,skewness_soma_non_ANML,by="somascan_id")
...
kurtosis_soma_non_ANML <- data.frame(kurtosis(somascan_non_ANML_log[,2:ncol(somascan_non_ANML_log)]))
names(kurtosis_soma_non_ANML) <- c("kurtosis_soma_non_ANML")
kurtosis_soma_non_ANML$somascan_id <- row.names(kurtosis_soma_non_ANML)
overlap_cor <- merge(overlap_cor,kurtosis_soma_non_ANML,by="somascan_id")
...
for (i in 1:nrow(overlap_cor)) {
  ...
  column <- pull(somascan_non_ANML_log,overlap_cor$somascan_id[i])
  overlap_cor$outlier_soma_non_ANML[i] <- sum((column < mean(column)-4*sd(column) | column > mean(column)+4*sd(column)))/nrow(somascan_non_ANML_log)
}
...
overlap_cor$below_lod_soma_non_ANML <- NA

for (i in 1:nrow(overlap_cor)) {
  overlap_cor$below_lod_soma_non_ANML[i] <- sum(somascan_non_ANML[[overlap_cor$somascan_id[i]]]<soma_lod$lod[soma_lod$somascan_id==overlap_cor$somascan_id[i]])/nrow(somascan_non_ANML)
}
...
overlap_coloc <- overlap_coloc[,c("uniprot_id","coloc_ANML_cis_tf","coloc_non_ANML_cis_tf","coloc_ANML_trans_tf","coloc_non_ANML_trans_tf")]
...
overlap_1_to_1_annot_non_ANML <- overlap_1_to_1_annot[,c(10,5,15:25,26,28,29,31,32,34,35:38,39,41,43,44:49,50:105)]
names(overlap_1_to_1_annot_non_ANML)
saveRDS(overlap_1_to_1_annot_non_ANML,"overlap_1_to_1_annot_non_ANML.RDS")
